...
const R3ViewContainerRef = class ViewContainerRef extends VE_ViewContainerRef {
  ...
  override insert(viewRef: ViewRef, index?: number): ViewRef {
    const detachedViewRefIdx = this._detachedViews.findIndex(
        detachedViewRef => (detachedViewRef as R3ViewRef<any>)._lView[ID] ===
            (viewRef as R3ViewRef<any>)._lView[ID]);
    if (detachedViewRefIdx !== -1) {
      removeFromArray(this._detachedViews, detachedViewRefIdx);
    }
